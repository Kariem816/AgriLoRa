...
static void rx_task(void *arg)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "rx_task: forwarding %d bytes to UART", pkt.length);

        uint8_t len = (uint8_t)pkt.length;
        uart_write_bytes(UART_PORT, &len, 1);
        uart_write_bytes(UART_PORT, (const char *)pkt.data, pkt.length);
    }
}
...
void app_main(void)
{
    rx_queue = xQueueCreate(QUEUE_DEPTH, sizeof(lora_packet_t));
    ...
    sx127x_rx_set_callback(lora_rx_callback, NULL, &device);
