...
static void tx_task(void *ctx) {
  ...
  Packet packet = {0};
  packet.header.version = 69;
  packet.header.from_node = 1;
  packet.header.to_node = 0;
  packet.header.sequence_number = 0;
  uint8_t payload[17] = "Hello, LoRa!";
  ...
  while (1) {
    ...
    if (ulTaskNotifyTakeIndexed(txDoneArrayIndex, pdTRUE,
